...
### 1.5 Header Emojis
Use Unicode emojis (e.g., 📦, 🏦) or MkDocs-compatible shortcodes (e.g., `:chart:`) in table headers.
...
*   **Key Detection:** Header must contain "現金股利".
*   **Features:** "Minimum Yield" custom filter.
*   **Filter Logic:** Script searches for column header containing "殖利率" AND "當日".
...
*   **Columns:** 8 Columns.
*   **Key Detection:** Header must contain "融資餘額".
*   **Features:** "Ratio" custom filter.
*   **Filter Logic:** Script searches for column header containing "比率".
*   **Format:**
    1.  `[**Code**](...)`
    2.  Name
    3.  Margin Balance (Num)
    4.  Close Price (Num)
    5.  Market Cap (Num - Custom parser for 兆/億)
    6.  Ratio (Percentage)
    7.  Risk (Emoji)
    8.  Date

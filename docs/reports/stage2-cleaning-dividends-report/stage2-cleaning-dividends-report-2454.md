---
authors: [wenchiehlee]
date: 2026-01-01
categories:
  - 股票
  - 股利分配
tags:
  - 股票
  - 股利
  - 殖利率
title: 股利分配報告 聯發科 (2454)
comments: false
draft: false
description: 股利分配報告 聯發科 (2454) - 自動產生
---

<style>
.sortable-table, .sortable-table td, .sortable-table th {
    font-size: var(--md-text-size) !important;
}
</style>

# :moneybag: 聯發科 (2454) 股利分配報告

!!! info "公司基本資訊"
    **:building_construction: 公司名稱**: 聯發科  
    **:identification_card: 股票代號**: 2454  
    **:calendar: 分析年度**: 2000-2026 (共 27 年)  
    **:clock3: 最新資料**: 2026 年  
    **🕒 更新時間**: 2026-01-01 20:28:22 CST

## :chart_with_upwards_trend: 快速摘要指標

| :chart: 指標 | :1234: 數值 | :traffic_light: 狀態 |
|:------------:|:----------:|:-------------------:|
| **分析年度總計** | 27 年 (2000-2026) | - |
| **最新現金股利** | 29.00 元 (2026) | - |
| **預測現金股利** | 60.78 元 (2027E) 📈 +109.6% | 🟡 中信心 |
| **現金殖利率5年平均** | 6.39% | 📊 市場水準 |
| **最新配發率** | 56.0% | 🟢 健康 |
| **連續配發年數** | 27 年 | ✅ 持續配發 |
| **股利穩定性評分** | 5.5/10 | ⚠️ 波動 |

---

## :bar_chart: 表三：殖利率比較矩陣

| :chart: 指標 | :new: 最新 | :two: 2年均 | :three: 3年均 | :five: 5年均 | :keycap_ten: 10年均 | :factory: 產業均 | :left_right_arrow: 相對產業 |
|:--------:|:------:|:------:|:------:|:------:|:--------:|:----------:|:-----------:|
| **現金殖利率(%)** | 4.23 | 4.08 | 4.25 | 6.39 | 4.78 | - | - |
| **總殖利率(%)** | 4.23 | 4.08 | 4.25 | 6.39 | 4.78 | - | - |
| **配發率(%)** | 56.0 | 68.3 | 83.4 | 91.1 | 86.4 | - | - |
| **填息天數(現金)** | - | - | - | 190 | 59 | - | - |

!!! abstract "殖利率評估"
    :chart: 最新殖利率：4.23%健康配發率(56.0%)顯示可持續性良好。

---

## :trophy: 股利穩定性評分明細

!!! success "總評分: 5.5/10 (⚠️ 波動)"
    本股票的股利穩定性總評分為 **5.5分** (滿分10分)，評級為 **⚠️ 波動**。

### :chart_with_upwards_trend: 評分拆解

| :trophy: 評分項目 | :1234: 得分 | :100: 滿分 | :chart: 百分比 | :information_source: 評估結果 |
|:----------------|:-------:|:------:|:---------:|:----------------|
| ✅ **配發一致性** | 4.0 | 4.0 | 100% | 27年發放 / 27年資料 (100.0%發放率) |
| ⚠️ **股利波動度** | 0.5 | 3.0 | 17% | CV=89.6% (非常波動) |
| 🟡 **配發率可持續性** | 1.0 | 2.0 | 50% | 平均83.4% (偏高但可持續) |
| ⚠️ **填息一致性** | 0.0 | 1.0 | 0% | 平均180天 (緩慢填息) |
| **:star: 總分** | **5.5** | **10.0** | **55%** | **⚠️ 波動** |

!!! info "評分說明"
    - **配發一致性(4分)**: 評估股利發放的連續性與穩定性
    - **股利波動度(3分)**: 使用變異係數(CV)衡量股利金額的波動程度
    - **配發率可持續性(2分)**: 評估配發率是否在健康範圍(30-70%理想)
    - **填息一致性(1分)**: 評估股價填息的速度與穩定性

---

## :seedling: 可持續性分析

### :key: 關鍵指標

- **最新配發率**: 56.0% → 🟢 **健康** (可持續範圍: <70%)
- **EPS涵蓋倍數**: 1.79x → 🟢 **充足** (最新股利 ÷ 最新EPS = 29.00 ÷ 51.77)
- **股利穩定性評分**: 5.5/10 → ⚠️ **波動**

### :chart_with_upwards_trend: 趨勢分析

- **5年趨勢**: 📊 混合 (表現波動)
- **近期變化(YoY)**: -46.3% (顯著下降)
- **配發組合**: 100% 現金 / 0% 股票 (以現金為主(對投資人友善))

---

## :crystal_ball: 股利預測

!!! tip "預測結果"
    **2027年預測現金股利 (加權平均)**: 60.78 元

    **預測區間**: 60.11 - 60.77 元

    **與2026年比較**: 📈 預期成長 109.6% (較2026年增加 31.78元)

### 情境分析表

| 情境 | 預測股利 | 計算可靠度 | 發生機率 | 期望值貢獻 | 條件假設 |
|:----:|:--------:|:----------:|:--------:|:----------:|:---------|
| 🟢 樂觀 | **60.11元** | 5.3/10 🟠 | **35%** | 21.04元 | Q4回升至14.24元 |
| 📊 基準 | **60.77元** | 8.3/10 🟢 | **30%** | 18.23元 | Q4維持14.96元 |
| 🔴 保守 | **60.77元** | 7.3/10 🟢 | **35%** | 21.27元 | Q4轉弱至14.96元 |
| **💡 加權** | **60.78元** | **-** | **100%** | **60.78元** | 綜合考慮各情境機率 |

### :clipboard: TTM 與情境計算明細

**最近 4 季 (用於 TTM 計算)**:

| 季度 | EPS (元) | 狀態 |
|:----:|:--------:|:----:|
| 2025-07 | 15.84 | ✅ 已公布 |
| 2025-04 | 17.50 | ✅ 已公布 |
| 2025-01 | 18.43 | ✅ 已公布 |
| 2024-10 | 14.96 | ✅ 已公布 (基準情境) |
| **TTM 合計** | **66.73** | - |

**歷史 Q4 (October) 趨勢**:

| 年度 | Q4 EPS (元) | 備註 |
|:----:|:-----------:|:----:|
| 2024 | 14.96 | ← 基準/保守 |
| 2023 | 16.17 |  |
| 2022 | 11.67 |  |
| 2021 | 19.03 |  |
| 2020 | 9.37 |  |
| **平均** | **14.24** | ← 樂觀情境 |

**情境推導邏輯**:

- 🟢 **樂觀情境**: Q4 = 14.24元 (依歷史水準或近期趨勢)
- 📊 **基準情境**: Q4 = 14.96元 (使用最近實際 Q4 值)
- 🔴 **保守情境**: Q4 = 14.96元 (維持基準或更弱)

!!! info "計算公式"
    - TTM EPS = Q1 + Q2 + Q3 + Q4(情境)
    - 預測股利 = TTM EPS × 平均配發率(5年) × 現金比例(5年)
    - 樂觀 Q4 (14.24) 來自上表近5年 Q4 平均值

### :chart_with_downwards_trend: 季度波動度分析

**各季度穩定性評估 (近10年)**:

| 季度 | 平均 EPS | 變異係數 | 評級 | 備註 |
|:----:|:--------:|:--------:|:----:|:-----|
| Q1 | 10.07 | 80.1% | 🚨 極度波動 | 最不穩定 |
| Q2 | 10.27 | 72.7% | ⚠️ 波動 | - |
| Q3 | 10.62 | 59.5% | ⚠️ 波動 | 最可預測 |
| Q4 | 9.04 | 67.7% | ⚠️ 波動 | - |

!!! info "Q4 預測參考說明"
    本公司 Q4 呈現中等波動 (變異係數 67.7%)，預測值僅供參考。
    建議結合公司營運狀況與產業趨勢綜合判斷。

### :calendar: FactSet 預測演化追蹤

| 預測時點 | N年EPS平均值 | vs 前次 | N+1年EPS平均值 | 備註 |
|---------|--------------|---------|---------------|------|
| 2025-08-18 | 66.78 | - | 78.79 | 26位分析師 |
| 2025-11-01 | 66.65 | -0.13 | 75.01 | 26位分析師 |
| 2025-12-27 | 66.78 | +0.13 | 78.79 | 26位分析師 |

**預測變化統計**:

- 總變化幅度: 66.78 → 66.78 (+0.00元, +0.0%)
- 最大單次下修: 2025-11-01 (-0.13元)
- 標準差: 0.06元 (相對穩定)

!!! info "FactSet 時間信心度評分"
    - 時間信心度: 9.0/10
    - 穩定性: 4/4
    - 更新頻率: 2/3
    - 覆蓋度: 3/3
    - 品質評分: 9.6/10

**FactSet vs TTM 方法比較**:

| 預測方法 | 預測EPS | 配息率 | 預估股利 | 差異 |
|---------|---------|--------|----------|------|
| TTM 方法 | 66.73 | 91% | 60.77 | - |
| FactSet 共識 | 66.78 | 91% | 60.81 | +0.05 (+0.1%) |

兩種方法預測結果**接近一致**，預測可靠度較高。

---

### :bar_chart: 信心度說明

**計算可靠度** vs **發生機率**:

- **計算可靠度**: 在該情境假設成立的前提下，計算方法的準確性
  - 基準情境 8.3/10 最高 → 完全基於實際已公布數據
  - 樂觀情境 5.3/10 較低 → 包含對未來的預測

- **發生機率**: 該情境實際發生的可能性
  - 樂觀情境 35% 最高

**建議**: 使用**加權平均 60.78元** 作為主要預測值，因為它綜合考慮了各情境的發生機率。

### :chart_with_upwards_trend: TTM EPS 信心度明細

| 維度 | 得分 | 說明 |
|------|------|------|
| 資料完整性 | 10.0/10 | ✅ 4個季度資料 |
| 資料時效性 | 7.0/10 | ⚠️ 最新季度落差 |
| 季度替代 | 10.0/10 | ✅ 季度連續性 |
| 異常偵測 | 10.0/10 | ✅ 季度一致性 |

**⚠️ 注意事項**:

- 最新季度尚未公布，使用 2025-07

### :gear: 預測方法

本預測採用 **TTM + FactSet 動態整合模型**：

#### 1️⃣ TTM 方法 (Trailing Twelve Months)

```
TTM EPS = 最近4季 EPS 總和
TTM 股利 = TTM EPS × 平均配發率(5年) × 現金比例(5年)
```

#### 2️⃣ FactSet 時間加權方法

```
時間權重 = exp(-0.1 × 天數差距/30)  # 越近期權重越高
時間加權EPS = Σ(EPS × 時間權重) / Σ(時間權重)
FactSet 股利 = 時間加權EPS × 平均配發率(5年) × 現金比例(5年)
```

#### 3️⃣ FactSet 時間信心度評分 (0-10分)

- **穩定性評分** (0-4分): 基於預測標準差
  - 標準差 < 1.0元 → 4分
  - 標準差 < 2.0元 → 3分
  - 標準差 < 3.0元 → 2分
  - 標準差 ≥ 3.0元 → 1分

- **更新頻率評分** (0-3分): 基於FactSet明細報告時間點數量
  - ≥5次更新 → 3分
  - ≥3次更新 → 2分
  - <3次更新 → 1分

- **分析師覆蓋評分** (0-3分): 基於分析師數量
  - ≥20位分析師 → 3分
  - ≥10位分析師 → 2分
  - <10位分析師 → 1分

#### 4️⃣ 動態整合邏輯

**本次採用: 100% FactSet 方法** 🎯

```
時間信心度 ≥ 9.0/10 且 品質評分 ≥ 8.5/10
→ W_FactSet = 1.0, W_TTM = 0.0
最終股利 = FactSet 股利
```

**決策說明**: 分析師共識穩定(標準差0.08元)且覆蓋充足(26位分析師, 3次更新)，完全採用FactSet時間加權預測

**本次計算結果**:

| 方法 | 股利預測 | 權重 | 貢獻 |
|------|----------|------|------|
| TTM | 60.77元 | 0.0% | 0.00元 |
| FactSet | 60.78元 | 100.0% | 60.78元 |
| **最終** | **60.78元** | **100%** | **60.78元** |

**情境分析**: 在最終股利基礎上套用樂觀/基準/保守情境

```
各情境股利 = 對應情境調整
最終預測 = Σ(各情境股利 × 發生機率)
```

!!! warning "重要提醒"
    - 本預測僅基於歷史資料統計與情境分析，**不構成投資建議**
    - 實際股利受多種因素影響：公司營運狀況、產業景氣、政策變化等
    - 情境機率為統計估計，實際結果可能與預測不同
    - 請以公司官方公告為準，本預測僅供參考

---

## :warning: 警示與觀察

!!! danger "關鍵警示"
    🔴 **警示**：2026 年股利削減 -46.3%

!!! success "正面訊號"
    🟢 **正面**：健康配發率 56.0% (可持續)

---

## :clipboard: 詳細股利歷史記錄

<table class="sortable-table">
<thead>
<tr>
<th>:calendar: 年度</th>
<th>:money_with_wings: 現金(元)</th>
<th>:chart: 股票(元)</th>
<th>:moneybag: 合計(元)</th>
<th>:chart_with_upwards_trend: 殖利率@除息價</th>
<th>:bar_chart: 殖利率@年均價</th>
<th>:arrow_up: 殖利率@最高</th>
<th>:arrow_down: 殖利率@最低</th>
<th>:repeat: 配發率</th>
<th>:1234: EPS(元)</th>
<th>:hourglass: 填息天數(現金)</th>
<th>:clock3: 填息天數(股票)</th>
<th>:traffic_light: 狀態</th>
</tr>
</thead>
<tbody>
<tr>
<td>2026</td>
<td>29.00</td>
<td>0.00</td>
<td>29.00</td>
<td>4.1%</td>
<td>4.2%</td>
<td>3.7%</td>
<td>5.1%</td>
<td>56%</td>
<td>51.77 (2025)</td>
<td>-</td>
<td>-</td>
<td>-</td>
</tr>
<tr>
<td>2025</td>
<td>54.00</td>
<td>0.00</td>
<td>54.00</td>
<td>4.0%</td>
<td>3.9%</td>
<td>3.4%</td>
<td>4.8%</td>
<td>81%</td>
<td>66.94 (2024)</td>
<td>-</td>
<td>-</td>
<td>-</td>
</tr>
<tr>
<td>2024</td>
<td>55.01</td>
<td>0.00</td>
<td>55.01</td>
<td>4.8%</td>
<td>4.6%</td>
<td>3.7%</td>
<td>6.3%</td>
<td>113%</td>
<td>48.52 (2023)</td>
<td>-</td>
<td>-</td>
<td>-</td>
</tr>
<tr>
<td>2023</td>
<td>76.01</td>
<td>0.00</td>
<td>76.01</td>
<td>9.7%</td>
<td>9.9%</td>
<td>7.2%</td>
<td>12.2%</td>
<td>102%</td>
<td>74.62 (2022)</td>
<td>62</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2022</td>
<td>73.00</td>
<td>0.00</td>
<td>73.00</td>
<td>9.1%</td>
<td>9.3%</td>
<td>6.0%</td>
<td>13.7%</td>
<td>103%</td>
<td>70.60 (2021)</td>
<td>317</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2021</td>
<td>37.00</td>
<td>0.00</td>
<td>37.00</td>
<td>3.9%</td>
<td>3.9%</td>
<td>3.1%</td>
<td>4.9%</td>
<td>142%</td>
<td>26.01 (2020)</td>
<td>9</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2020</td>
<td>10.50</td>
<td>0.00</td>
<td>10.50</td>
<td>1.8%</td>
<td>1.9%</td>
<td>1.4%</td>
<td>3.9%</td>
<td>71%</td>
<td>14.69 (2019)</td>
<td>1</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2019</td>
<td>9.00</td>
<td>0.00</td>
<td>9.00</td>
<td>2.8%</td>
<td>2.7%</td>
<td>1.9%</td>
<td>4.2%</td>
<td>68%</td>
<td>13.26 (2018)</td>
<td>4</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2018</td>
<td>10.00</td>
<td>0.00</td>
<td>10.00</td>
<td>3.5%</td>
<td>3.6%</td>
<td>2.7%</td>
<td>5.0%</td>
<td>64%</td>
<td>15.56 (2017)</td>
<td>4</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2017</td>
<td>9.50</td>
<td>0.00</td>
<td>9.50</td>
<td>3.6%</td>
<td>3.7%</td>
<td>2.7%</td>
<td>4.7%</td>
<td>63%</td>
<td>15.16 (2016)</td>
<td>16</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2016</td>
<td>11.00</td>
<td>0.00</td>
<td>11.00</td>
<td>4.5%</td>
<td>4.8%</td>
<td>4.2%</td>
<td>5.7%</td>
<td>66%</td>
<td>16.60 (2015)</td>
<td>9</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2015</td>
<td>22.00</td>
<td>0.00</td>
<td>22.00</td>
<td>5.7%</td>
<td>6.2%</td>
<td>4.4%</td>
<td>9.7%</td>
<td>73%</td>
<td>30.09 (2014)</td>
<td>1021</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2014</td>
<td>15.00</td>
<td>0.00</td>
<td>15.00</td>
<td>2.8%</td>
<td>3.2%</td>
<td>2.8%</td>
<td>3.9%</td>
<td>73%</td>
<td>20.51 (2013)</td>
<td>1461</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2013</td>
<td>9.00</td>
<td>0.00</td>
<td>9.00</td>
<td>2.5%</td>
<td>2.5%</td>
<td>2.0%</td>
<td>3.0%</td>
<td>68%</td>
<td>13.30 (2012)</td>
<td>9</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2012</td>
<td>9.00</td>
<td>0.00</td>
<td>9.00</td>
<td>3.6%</td>
<td>3.1%</td>
<td>2.6%</td>
<td>3.8%</td>
<td>72%</td>
<td>12.45 (2011)</td>
<td>2</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2011</td>
<td>20.00</td>
<td>0.00</td>
<td>20.00</td>
<td>6.6%</td>
<td>6.3%</td>
<td>4.7%</td>
<td>9.1%</td>
<td>70%</td>
<td>28.56 (2010)</td>
<td>50</td>
<td>-</td>
<td>✅</td>
</tr>
<tr>
<td>2010</td>
<td>26.00</td>
<td>0.02</td>
<td>26.02</td>
<td>5.4%</td>
<td>5.4%</td>
<td>4.4%</td>
<td>7.0%</td>
<td>76%</td>
<td>34.28 (2009)</td>
<td>29</td>
<td>29</td>
<td>✅</td>
</tr>
<tr>
<td>2009</td>
<td>14.00</td>
<td>0.02</td>
<td>14.02</td>
<td>3.1%</td>
<td>3.4%</td>
<td>2.5%</td>
<td>6.1%</td>
<td>77%</td>
<td>18.27 (2008)</td>
<td>1</td>
<td>1</td>
<td>✅</td>
</tr>
<tr>
<td>2008</td>
<td>19.00</td>
<td>0.10</td>
<td>19.10</td>
<td>6.6%</td>
<td>5.8%</td>
<td>4.3%</td>
<td>10.7%</td>
<td>57%</td>
<td>33.69 (2007)</td>
<td>3</td>
<td>3</td>
<td>✅</td>
</tr>
<tr>
<td>2007</td>
<td>15.00</td>
<td>0.50</td>
<td>15.50</td>
<td>2.4%</td>
<td>3.1%</td>
<td>2.3%</td>
<td>4.6%</td>
<td>130%</td>
<td>11.91 (2006)</td>
<td>63</td>
<td>63</td>
<td>✅</td>
</tr>
<tr>
<td>2006</td>
<td>11.00</td>
<td>1.00</td>
<td>12.00</td>
<td>3.7%</td>
<td>3.3%</td>
<td>2.6%</td>
<td>4.2%</td>
<td>103%</td>
<td>11.65 (2005)</td>
<td>11</td>
<td>11</td>
<td>✅</td>
</tr>
<tr>
<td>2005</td>
<td>10.00</td>
<td>1.00</td>
<td>11.00</td>
<td>2.7%</td>
<td>3.6%</td>
<td>2.5%</td>
<td>5.8%</td>
<td>314%</td>
<td>3.50 (2004)</td>
<td>82</td>
<td>82</td>
<td>✅</td>
</tr>
<tr>
<td>2004</td>
<td>8.55</td>
<td>1.81</td>
<td>10.36</td>
<td>3.7%</td>
<td>3.1%</td>
<td>2.2%</td>
<td>4.6%</td>
<td>135%</td>
<td>7.69 (2003)</td>
<td>17</td>
<td>17</td>
<td>✅</td>
</tr>
<tr>
<td>2003</td>
<td>8.00</td>
<td>3.50</td>
<td>11.50</td>
<td>1.9%</td>
<td>2.5%</td>
<td>1.8%</td>
<td>3.2%</td>
<td>141%</td>
<td>8.17 (2002)</td>
<td>677</td>
<td>677</td>
<td>✅</td>
</tr>
<tr>
<td>2002</td>
<td>4.00</td>
<td>4.00</td>
<td>8.00</td>
<td>0.9%</td>
<td>0.8%</td>
<td>0.5%</td>
<td>1.7%</td>
<td>94%</td>
<td>8.54 (2001)</td>
<td>226</td>
<td>226</td>
<td>✅</td>
</tr>
<tr>
<td>2001</td>
<td>1.00</td>
<td>4.00</td>
<td>5.00</td>
<td>0.3%</td>
<td>0.3%</td>
<td>0.2%</td>
<td>0.5%</td>
<td>168%</td>
<td>2.98 (2000)</td>
<td>56</td>
<td>56</td>
<td>✅</td>
</tr>
<tr>
<td>2000</td>
<td>0.00</td>
<td>7.50</td>
<td>7.50</td>
<td>-</td>
<td>-</td>
<td>-</td>
<td>-</td>
<td>-</td>
<td>-</td>
<td>-</td>
<td>-</td>
<td>-</td>
</tr>
</tbody>
</table>

**圖例**：✅ 已填息 | ⏳ 部分填息 | ❌ 未填息 | - 無資料

---

## :information_source: 資料來源與方法

!!! note "資料來源說明"
    - **主要來源**：`cleaned_dividends.csv` (Type 1: DividendDetail)
    - **EPS資料**：交叉比對 `cleaned_performance1.csv` (Type 7: Quarterly Performance)
    - **FactSet預測**：`factset_detailed_report_latest.csv` (Type 51: FactSet Analyst Consensus Summary)
    - **預測演化**：FactSet 詳細報告時間序列 (同上 CSV)

!!! info "報告元資訊"
    - **報告產生時間**：2026-01-01 20:28:22
    - **資料品質評分**：9.7/10
    - **分析期間**：2000 - 2026 (共 27 年)
    - **資料來源**：Stage 2 資料清理股利報告系統

---

<div class="result" markdown>

:material-information-outline: **本報告僅供參考，投資決策請審慎評估**

</div>
